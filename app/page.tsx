...
export default function Home() {
  ...
  const [section3ModalPhotoIndex, setSection3ModalPhotoIndex] = useState<
    number | null
  >(null);
  const section3GalleryViewportRef = useRef<HTMLDivElement | null>(null);
  ...
  useEffect(() => {
    const onWheel = (e: WheelEvent) => {
      ...
      if (section === 2 && section3ActiveCategoryId && !isSection3ModalOpen) {
        const scrollDistance = Math.max(
          150,
          Math.min(360, Math.abs(e.deltaY) * 1.15),
        );

        if (scrollExpandedGallery(direction, scrollDistance)) {
          e.preventDefault();
          return;
        }
  ...
  }, [isSection3ModalOpen, navigateBetweenSections, section, section3ActiveCategoryId]);
        ...
        {section === 2 && (
          ...
          >
            <Section3
              ...
              galleryViewportRef={section3GalleryViewportRef}
              onGalleryProgressChange={setSection3GalleryProgress}
